<?php ?>

<link rel="stylesheet" href="Style/MenuLateral.css">

<aside id="menu">
    <div id="divLogo">
        <h1 class="mb-0" id="WORK">WORK</h1>
        <h1 class="mb-0" id="ETHER">ETHER</h1>
    </div>
    <button class="btnAside" id="btnProjetos" name="Projetos">
        <img src="Icones/Projetos.png" alt="">
        <p class="mb-0">Projetos</p>
    </button>
    <button class="btnAside" id="btnTarefas" name="Tarefas">
        <img src="Icones/Tarefas.png" alt="">
        <p class="mb-0">Tarefas</p>
    </button>
    <button class="btnAside" id="btnConversas" name="Conversas">
        <img src="Icones/Conversas.png" alt="">
        <p class="mb-0">Conversas</p>
    </button>
    <button class="btnAside" id="btnAmigos" name="Amigos">
        <img src="Icones/Amigos.png" alt="">
        <p class="mb-0">Amigos</p>
    </button>
    <button class="btnAside" id="btnPerfil" name="Perfil">
        <img src="Icones/Perfil.png" alt="">
        <p class="mb-0">Perfil</p>
    </button>
</aside>

<script src="Script/MenuLateral.js"></script>
